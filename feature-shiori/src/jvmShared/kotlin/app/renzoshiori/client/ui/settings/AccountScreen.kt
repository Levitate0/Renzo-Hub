...
import app.renzoshiori.client.ui.util.rememberImagePicker
import app.renzoshiori.client.ui.util.rememberOfflineFolderPicker
...
fun AccountScreen(
    ...
    val store = ShioriRuntime.app.offlineStore
    var folderLabel by remember { mutableStateOf(store.folderLabel()) }
    val folderPicker = rememberOfflineFolderPicker { label ->
        if (label != null) folderLabel = label
    }
    ...
    val avatarPicker = rememberImagePicker { bytes, mime ->
        if (bytes != null) {
            when (val loaded = validatePickedAvatar(bytes, mime)) {
                is AvatarLoad.Ok -> {
    ...
    val sections = listOf(
        ...
        // Web: MonitorSmartphone — the remembered sign-ins on this account.
        SettingsNavSection("devices", "Devices", Icons.Filled.Devices),
    ...
    SettingsScaffold(title = "Account", onBack = onBack, snackbar = snackbar) { padding ->
        Column(
            ...
            when (activeSection) {
                ...
                "devices" -> SettingsCard(
                    title = "Devices",
                    description = "Browsers and televisions that stay signed in to your account. " +
                        "Sign out anything you don't recognise — or any TV you no longer use.",
                ) {
