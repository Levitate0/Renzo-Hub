...
fun LoginScreen(
    ...
    /** Return to the Connect step — the escape from a wrong server's login. */
    ...
    // screen and approved elsewhere, rather than spelling a password out with
    // a D-pad. The password form is still one press away for anyone who wants
    // it, and for servers that have no pairing endpoints.
    val isTv = app.renzoshiori.client.ui.tv.LocalIsTv.current
    var pairing by remember(isTv) { mutableStateOf(isTv) }
    if (pairing) {
        TvPairingScreen(
            serverUrl = step.serverUrl,
            deviceName = top.levitatemedia.renzo.hub.core.HubPlatform.deviceName,
            onPaired = { token, user, username -> vm.acceptPairedSession(token, user, username) },
