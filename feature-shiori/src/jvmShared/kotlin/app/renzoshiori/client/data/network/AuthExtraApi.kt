...
interface AuthExtraApi {
    ...
    // ── Remembered devices (authenticated; call via currentServiceOf) ──
    // Unlike the endpoints above these require the signed-in client: the
    // Authorization header comes from the auth interceptor, and `isCurrent`
    // is derived from the refresh cookie the cookie jar presents.

    @GET("api/auth/devices")
    suspend fun listDevices(): List<RememberedDeviceDto>

    @DELETE("api/auth/devices/{id}")
    suspend fun revokeDevice(@Path("id") id: String): GenericResultDto
}
