...
private fun AppRoot(app: AppServices, onSwitchApp: (() -> Unit)?, onSessionLost: () -> Unit) {
    ...
    var drawerOpen by remember {
        mutableStateOf(app.demoOverlayDrawer)
    }
    var accountMenuOpen by remember {
        mutableStateOf(app.demoOverlayAccount)
    }
    ...
    RenzoBackHandler(enabled = nav.stack.size > 1) { nav.back() }
    ...
    when (screen) {
        ...
        is Screen.Player -> RenzoPlayerRoute(
            app = app,
            titleId = screen.titleId,
            ep = screen.ep,
            titleName = screen.titleName,
            onExit = { nav.back() },
            onSwitchEpisode = { newEp: Int -> nav.replaceTop(Screen.Player(screen.titleId, newEp, screen.titleName)) },
            onSwitchTitle = { newId: Int, newEp: Int, name: String -> nav.replaceTop(Screen.Player(newId, newEp, name)) },
        )
